# 05.6_advanced_strategy.R
# Multi-Asset Advanced Reversion Strategy (Portfolio-Level Backtest) - Fixed Risk & Volume Rules
...
days_since_t1 <- rep(NA_integer_, n_symbols) # Time-based exit state
...
for (i in 201:length(dates)) {
  ...
  # Update days since Target 1 for active positions
  for (j in active_idx) {
    if (position[j] == 0.5 && !is.na(days_since_t1[j])) {
      days_since_t1[j] <- days_since_t1[j] + 1
    }
  }
    ...
    if (position[j] > 0) {
      ...
      if (L_today[j] <= stop_loss[j]) {
        ...
        if (position[j] == 1 && H_today[j] >= target1[j]) {
          ...
          days_since_t1[j] <- 0 # Initialize counter for Time-based exit
        ...
        # Check Target 2 Exits (Price or Time)
        if (position[j] == 0.5) {
          # Time-Based Exit (Rule 1): > 21 days since T1
          if (days_since_t1[j] >= 21) {
            exec_price_t2 <- C_today[j] # Market close
            global_cash <- global_cash + (shares[j] * exec_price_t2)
            trade_log[[length(trade_log) + 1]] <- data.frame(Date = current_date, Symbol = sym, Action = "TIME_EXIT", Price = exec_price_t2, Shares = -shares[j])
            shares[j] <- 0
            position[j] <- 0
            days_since_t1[j] <- NA
          } 
          # SMA 200 Reversion Exit
          else if (H_today[j] >= sma200[j]) {
...
p <- ggplot(plot_data, aes(x = Date, y = Equity)) +
  ...
  labs(title = "Portfolio Advanced Reversal Strategy",
       subtitle = "100 Companies (Even Distribution, 21-Day Time Exit)",
       y = "Portfolio Equity ($)",
       x = "Date")

print(p)
ggsave("portfolio_advanced_equity_rules.png", plot = p, width = 10, height = 6)
cat("Multi-asset backtest complete! Results saved to portfolio_advanced_equity_rules.png\n")
