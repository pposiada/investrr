# 14_savings_10yr_ath_stop_pl.R
# Advanced Reversion Strategy - 10-Year Savings with ATH Trailing Stop (2016-2026) - Polish Market
    ...
    for (j in active_idx) {
      ...
      if (shares[j] > 0 && strategy_type %in% c("Single_Entry", "Multiple_Entries")) {
        ...
        if (stop_loss_active[j]) {
          ...
          current_stop <- highest_price_since_activation[j] * 0.98
      ...
      if (global_cash > 0 && length(triggered_indices) > 0) {
        ...
        for (j in triggered_indices) {
          ...
          if (alloc_shares > 0) {
            ...
            shares[j] <- shares[j] + alloc_shares
            setup_armed[j] <- FALSE
            # Note: stop_loss_active[j] and highest_price_since_activation[j] remain FALSE/NA on new entry
          }
        }
      }
    }
...
cat("Running Single Entry Reversion (with -2% Trail after ATH)...\n")
res_single <- run_savings_backtest("Single_Entry")

cat("Running Multiple Entries Reversion (with -2% Trail after ATH)...\n")
...
cat("\n========================================================================\n")
cat("10-Year Savings Backtest Results with ATH -2% Stop (2016-2026):\n")
...
p <- ggplot(plot_data, aes(x = Date, y = Equity, color = Strategy)) +
  ...
  theme_minimal() +
  labs(title = "10-Year Savings Performance with ATH -2% Stop - Polish Market",
       subtitle = "Saving 2,000 PLN/month starting with 0 PLN. Exit at -2% stop after hitting ATH.",
...
ggsave("outputs/portfolio_savings_10yr_ath_stop_pl.png", plot = p, width = 12, height = 7)
cat("Done! Chart saved to outputs/portfolio_savings_10yr_ath_stop_pl.png\n")
