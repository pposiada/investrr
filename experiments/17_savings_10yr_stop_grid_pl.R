# 17_savings_10yr_stop_grid_pl.R
# Advanced Reversion Strategy - 10-Year Savings fixed Stop Loss Grid Search - Polish Market
    ...
    if (global_cash > 0 && length(triggered_indices) > 0) {
      ...
      for (j in triggered_indices) {
        ...
        if (alloc_shares > 0) {
          ...
          # Update entry price and active stop loss (-X% from entry point)
          if (shares[j] == 0) {
...
cat("\n========================================================================\n")
cat("Grid Search Results: Fixed Stop Loss Optimization (10-Year PL):\n")
print(as.data.frame(summary_df %>% arrange(desc(Final_Equity))))
cat("========================================================================\n")

if(!dir.exists("outputs")) dir.create("outputs")
write.csv(summary_df, "outputs/portfolio_savings_10yr_stop_grid_summary.csv", row.names = FALSE)
...
p <- ggplot(summary_df, aes(x = factor(Stop_Loss_Pct), y = Final_Equity, fill = Strategy)) +
  ...
  theme_minimal() +
  labs(title = "Fixed Stop Loss Optimization (From Entry) - 10-Year Savings",
       subtitle = "Comparing Stop Pct (1% to 20%) in Single & Multiple Entry Models",
       y = "Final Portfolio Equity (PLN)",
       x = "Stop Loss Percentage from Entry (%)",
       fill = "Strategy Type") +
  theme(legend.position = "bottom") +
  scale_y_continuous(labels = scales::dollar_format(prefix = "", suffix = " PLN"))

ggsave("outputs/portfolio_savings_10yr_stop_grid_pl.png", plot = p, width = 12, height = 7)
cat("Done! Chart saved to outputs/portfolio_savings_10yr_stop_grid_pl.png\n")
