# 15_savings_rolling_6yr_ath_stop_15pct_pl.R
# Advanced Reversion Strategy - 6-Year Rolling Periods Savings Backtest with 15% ATH Trailing Stop - Polish Market
...
run_period_savings_backtest <- function(p_name, p_start, p_end, strategy_type, stop_loss_pct = 0.15) {
  ...
  for (i in 1:length(dates)) {
    ...
    for (j in active_idx) {
      ...
      # Exit logic for active positions in Indicator Strategies
    ...
    if (strategy_type == "Blind_DCA_Monthly") {
      ...
      # Benchmark DCA WIG20: Invest the 2000 PLN savings equally among WIG20 core tickers on the first day of each month
      if (current_month != prev_month) {
...
# 5. Execute Loop
all_results <- list()
strategies <- c("Blind_DCA_Monthly", "WIG20_DCA_Monthly", "Single_Entry", "Multiple_Entries")

for (p in periods) {
  for (strat in strategies) {
    cat("Running savings backtest for period:", p$name, "| Strategy:", strat, "...\n")
    res <- run_period_savings_backtest(p$name, p$start, p$end, strat, stop_loss_pct = 0.15)
    if (!is.null(res)) {
...
cat("\n========================================================================\n")
cat("Final Equities & Contributions (PLN) by 6-Year Rolling Period & Strategy (15% ATH Stop):\n")
print(as.data.frame(summary_table))
cat("========================================================================\n")

if(!dir.exists("outputs")) dir.create("outputs")
write.csv(summary_table, "outputs/portfolio_rolling_6yr_savings_ath_stop_15pct_summary.csv", row.names = FALSE)
...
p <- ggplot(plot_data, aes(x = TradingDay, y = Equity, color = Strategy)) +
  ...
  theme_minimal() +
  labs(title = "Rolling 6-Year Savings Backtest with 15% Post-ATH Trailing Stop - Polish Market",
       subtitle = "Accumulating stocks (2,000 PLN/month) starting at 0 PLN. Exit at -15% stop after hitting ATH.",
...
ggsave("outputs/portfolio_rolling_6yr_savings_ath_stop_15pct_pl.png", plot = p, width = 14, height = 9)
cat("Done! Chart saved to outputs/portfolio_rolling_6yr_savings_ath_stop_15pct_pl.png\n")
