# 12_savings_2000_2026_backtest_pl.R
# Advanced Reversion Strategy - Long-term Savings Backtest (2000-2026) - Polish Market
...
run_savings_backtest <- function(strategy_type) {
  ...
  for (i in 1:length(dates)) {
    ...
    if (strategy_type == "Blind_DCA_Monthly") {
      ...
      # WIG20 DCA: Invest the 2000 PLN savings equally among the 4 historical blue-chip pillars (PEO.WA, KGH.WA, PKN.WA, OPL.WA)
      if (current_month != prev_month) {
...
cat("Running Single Entry Reversion...\n")
res_single <- run_savings_backtest("Single_Entry")

cat("Running Multiple Entries Reversion...\n")
...
cat("\n========================================================================\n")
cat("26-Year Savings Backtest Results (2000-2026) - Polish Market:\n")
...
p <- ggplot(plot_data, aes(x = Date, y = Equity, color = Strategy)) +
  ...
  theme_minimal() +
  labs(title = "26-Year Savings Strategy Performance - Polish Market (2000-2026)",
       subtitle = "Saving 2,000 PLN/month starting with 0 PLN. Holding positions indefinitely.",
...
ggsave("outputs/portfolio_savings_2000_2026_pl.png", plot = p, width = 12, height = 7)
cat("Done! Chart saved to outputs/portfolio_savings_2000_2026_pl.png\n")
