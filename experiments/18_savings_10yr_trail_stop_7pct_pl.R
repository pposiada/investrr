# 18_savings_10yr_trail_stop_7pct_pl.R
# Advanced Reversion Strategy - 10-Year Savings with 7% Trailing Stop Loss (2016-2026) - Polish Market
...
run_savings_backtest <- function(strategy_type) {
  ...
  highest_price <- rep(NA_real_, n_symbols)
  ...
  for (i in 1:length(dates)) {
    ...
    for (j in active_idx) {
      ...
      # Exit logic for active positions: Trailing Stop Loss
      if (shares[j] > 0 && strategy_type %in% c("Single_Entry", "Multiple_Entries")) {
        # Update peak price since entry
        highest_price[j] <- max(H_today[j], highest_price[j], na.rm = TRUE)
        # Update trailing stop price
        stop_loss[j] <- highest_price[j] * 0.93
      ...
      if (global_cash > 0 && length(triggered_indices) > 0) {
        ...
        for (j in triggered_indices) {
          ...
          if (alloc_shares > 0) {
            ...
            # Update entry price, peak price, and trailing stop loss (-7% trailing)
            if (shares[j] == 0) {
              entry_price[j] <- entry_px
              highest_price[j] <- entry_px
            } else {
              entry_price[j] <- (shares[j] * entry_price[j] + alloc_shares * entry_px) / (shares[j] + alloc_shares)
              highest_price[j] <- max(H_today[j], entry_price[j], na.rm = TRUE)
            }
            stop_loss[j] <- highest_price[j] * 0.93
...
cat("Running Single Entry Reversion (with 7% Trail Stop)...\n")
res_single <- run_savings_backtest("Single_Entry")

cat("Running Multiple Entries Reversion (with 7% Trail Stop)...\n")
...
cat("\n========================================================================\n")
cat("10-Year Savings Backtest Results with 7% Trailing Stop Loss (2016-2026):\n")
...
p <- ggplot(plot_data, aes(x = Date, y = Equity, color = Strategy)) +
  ...
  theme_minimal() +
  labs(title = "10-Year Savings Performance with 7% Trailing Stop - Polish Market",
       subtitle = "Saving 2,000 PLN/month starting with 0 PLN. Exit at 7% trailing stop-loss.",
...
ggsave("outputs/portfolio_savings_10yr_trail_stop_7pct_pl.png", plot = p, width = 12, height = 7)
cat("Done! Chart saved to outputs/portfolio_savings_10yr_trail_stop_7pct_pl.png\n")
