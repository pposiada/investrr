# 05.2_advanced_strategy.R
# Multi-Asset Advanced Reversion Strategy (VIX Filter Rule)
...
cat("Fetching VIX data for macro panic filter...\n")
vix_data <- tq_get("^VIX", get = "stock.prices", from = "2018-01-01", to = "2026-12-31") %>%
...
# Align VIX data with the main date array
vix_aligned <- data.frame(date = dates) %>%
  left_join(vix_data, by = "date") %>%
  tidyr::fill(VIX_Close, .direction = "down")
VIX_mat <- vix_aligned$VIX_Close
...
for (i in 201:length(dates)) {
  ...
  for (j in active_idx) {
    ...
    if (position[j] > 0) {
      ...
      if (L_today[j] <= stop_loss[j]) {
        setup_armed[j] <- FALSE
      } else if (!is.na(vix_today) && vix_today <= 35 && C_today[j] > sma20[j] && C_today[j] < sma200[j]) {
...
p <- ggplot(plot_data, aes(x = Date, y = Equity)) +
  ...
  theme_minimal() +
  labs(title = "Portfolio Advanced Reversal Strategy (VIX Filter)",
       subtitle = "100 Systemically Important Companies",
       y = "Portfolio Equity ($)",
       x = "Date")
print(p)
ggsave("portfolio_advanced_equity_vix_filter.png", plot = p, width = 10, height = 6)
cat("Results saved to portfolio_advanced_equity_vix_filter.png\n")
